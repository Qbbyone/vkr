...
class Ppg_analyzer:
    ...
    def fiducial_points(self, x, pks, fs, vis):
        ...
        for i in range(len(ons) - 1):
            ...
            if len(aux_dic) != 0:
                (ind_max,) = np.where(
                    ibi_2d_portion[aux_dic] == np.max(ibi_2d_portion[aux_dic])
                )
        ...
        for i in range(len(ons) - 1):
            ...
            if len(aux_m1d) != 0:
                (ind_max,) = np.where(
                    ibi_1d_portion[aux_m1d] == np.max(ibi_1d_portion[aux_m1d])
                )
        ...
        for i in range(len(ons) - 1):
            ...
            aux_tip = np.round(
                ((ibi_portion[0] - ibi_portion[ind_m1d]) / ibi_1d_portion[ind_m1d])
                + ind_m1d
            )
        ...
        for i in range(len(ons) - 1):
            ...
            # a point:
            (ind_a,) = np.where(
                ibi_2d_portion[aux_m2d_pks] == np.max(ibi_2d_portion[aux_m2d_pks])
            )
            ...
            if ind_a < ind_m1d:
                ...
                # b point:
                ind_b = np.where(
                    ibi_2d_portion[aux_m2d_ons] == np.min(ibi_2d_portion[aux_m2d_ons])
                )
            ...
            if len(ind_e) >= 1:
                ...
                if len(ind_c) != 0:
                    ...
                    (ind_c,) = np.where(
                        ibi_2d_portion[ind_c_aux] == np.max(ibi_2d_portion[ind_c_aux])
                    )
                ...
                # d point:
                if len(ind_c) != 0:
                    ind_d = np.intersect1d(
                        np.where(aux_m2d_ons < ind_e), np.where(aux_m2d_ons > ind_c)
                    )
                    if len(ind_d) != 0:
                        ind_d_aux = aux_m2d_ons[ind_d]
                        (ind_d,) = np.where(
                            ibi_2d_portion[ind_d_aux]
                            == np.min(ibi_2d_portion[ind_d_aux])
                        )
            ...
            if len(aux_p3d_ons) != 0 and len(ind_c) != 0 and len(ind_d) != 0:
                ...
                if len(ind_dic) != 0:
                    ...
                    if ind_p2 > ind_dic - start:
                        ind_between = np.intersect1d(
                            np.where(aux_x_pks < ind_p2),
        ...
        if vis == True:
            fig, (ax1, ax2, ax3, ax4) = plt.subplots(
                4, 1, sharex=True, sharey=False, figsize=(10, 10)
            )
    ...
    def detect_beats(self):
        ...
        if self.gr:
            ...
            ax3.scatter(
                t[0] + ibis / self.fs, self.ppg_filt[ibis], color="orange", marker="o"
            )
            ax3.axis("tight")
    ...
    def calc_features(self):
        ...
        for beat_no in range(len(self.fidp["dia"])):
            delta_t[beat_no] = (
                self.fidp["dia"][beat_no] - self.fidp["pks"][beat_no]
            ) / self.fs
        ...
        for beat_no in range(len(self.fidp["dia"])):
            agi[beat_no] = (
                self.fidp["bmag2d"][beat_no]
                - self.fidp["cmag2d"][beat_no]
                - self.fidp["dmag2d"][beat_no]
                - self.fidp["emag2d"][beat_no]
            ) / self.fs
